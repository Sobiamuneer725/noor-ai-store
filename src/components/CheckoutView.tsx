...
import { CheckCircle, ShieldCheck, Lock, Truck, CreditCard, ArrowLeft } from 'lucide-react';
      ...
      <div className="max-w-2xl mx-auto py-16 text-center space-y-6 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="space-y-2">
          <span className="text-xs uppercase font-bold tracking-widest text-emerald-700 bg-emerald-50 px-3.5 py-1 rounded-full border border-emerald-200 inline-flex items-center gap-1.5 shadow-2xs">
            <span>Order Confirmed • #{orderId || 'NOOR-SUCCESS'}</span>
          </span>
          <h1 className="text-3xl font-extrabold text-neutral-900 mt-2">
            Thank You for Shopping at Noor ul Haya!
          </h1>
          <p className="text-neutral-600 max-w-md mx-auto">
            Your order has been placed successfully. We'll reach out on WhatsApp to confirm delivery details.
          </p>
        </div>
        ...
        <div className="bg-neutral-50 p-6 rounded-2xl border border-neutral-200 text-left space-y-3">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-extrabold text-[#A6813C]">Rs {total.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-4 pt-4">
          <button
            ...
            className="px-6 py-3 bg-[#171310] hover:bg-[#2A2319] text-[#D4AF6A] font-semibold rounded-xl transition-colors shadow-sm"
          >
            Return to Catalog
          </button>
          <button
  ...
  return (
    <div className="space-y-8 pb-16">
      <div className="flex items-center justify-between border-b border-neutral-200 pb-4">
        <div>
          <h1 className="text-3xl font-extrabold text-neutral-900 tracking-tight">Checkout</h1>
          <p className="text-sm text-neutral-500">Complete your shipping details</p>
        </div>
        <button
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-neutral-200 shadow-2xs space-y-4">
            <h3 className="text-lg font-bold text-neutral-900 flex items-center space-x-2">
              <Truck className="w-5 h-5 text-[#A6813C]" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-[#A6813C] focus:outline-none"
                />
              </div>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-neutral-200 shadow-2xs space-y-4">
            <h3 className="text-lg font-bold text-neutral-900 flex items-center space-x-2">
              <CreditCard className="w-5 h-5 text-[#A6813C]" />
              <span>Payment Method</span>
            </h3>

            <div className="p-4 bg-[#F5EFE3] border border-[#E7DFCF] rounded-2xl flex items-center space-x-3 text-[#171310] text-sm font-medium">
              <Lock className="w-5 h-5 text-[#A6813C] shrink-0" />
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-neutral-200 shadow-sm space-y-6 h-fit sticky top-24">
          ...
          <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
            {items.map(({ product, quantity, selectedColor }) => (
              <div key={`${product.id}__${selectedColor || ''}`} className="flex items-center justify-between text-sm">
                <span className="text-neutral-600 truncate max-w-[160px]">
                  {quantity}x {product.name}
                  {selectedColor && <span className="text-neutral-400"> ({selectedColor})</span>}
                </span>
                <span className="font-semibold text-neutral-900">Rs {(product.price * quantity).toLocaleString()}</span>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2 border-t border-neutral-200 pt-4 text-sm">
            ...
            <div className="flex justify-between text-neutral-500">
              ...
              {shipping === 0 ? (
                ...
                <span className="font-semibold text-neutral-900">Rs {shipping.toLocaleString()}</span>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-between items-center text-lg font-extrabold border-t border-neutral-200 pt-4">
            ...
            <span className="text-2xl text-[#A6813C]">Rs {total.toLocaleString()}</span>
          </div>

          <button
            type="submit"
            disabled={processing || items.length === 0}
            className="w-full py-4 bg-[#171310] hover:bg-[#2A2319] text-[#D4AF6A] font-bold rounded-2xl shadow-lg transition-all flex items-center justify-center space-x-2"
          >
            {processing ? (
